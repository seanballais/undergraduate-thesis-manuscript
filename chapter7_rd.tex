...
Each data set used in the experiments uses differently-sized bounding regions. For SFLP-II, a $12x12$ bounding region is used. mSFLIP uses a $260x260$ bounding region, while mKra30a uses a $250x250$ one. Notice that $c = 2$ provides the best average fitness for SFLP-II. On the other hand, for mKra30a, $c = 8$ provides the best fitness. Lastly, the best average fitness is given for mSFLP-III when $c = 12$. From this, we can say that the larger the bounding region, the larger the $c$ parameter values must be in order for us to obtain the best results as much as possible. % TODO: Contrinue. We can attribute this to the fact that the parameter determines how much the estimated location of a go Hence, the $c$ parameter affects the performance of our proposed GWO approach. Later research can focus on determining the ideal $c$ value for a certain bounding region size and identifying whether it can be mathematically modelled instead, rather than being a parameter.

% TODO: Add line graph of size and c value.
