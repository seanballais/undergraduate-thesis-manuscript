...
\begin{figure}[h!]
	...
	\caption{A visualization of a solution where the buildings tend to move towards the axes, with many already being restricted to them.}
...
We also have another small but still important modification to the classical GWO. In each iteration, the global best generated by our algorithm is tracked by our approach. The global best, however, does not replace the alpha wolf of any iteration, even if the alpha wolf has already become infeasible. The algorithm continues as though no tracking is being conducted in the first place.
...
\State Set $\vec{X}_{best}$ to be the global best wolf.
\While{t $<$ T}
	...
	\If {$\vec{X}_{best}$ == null or $f(\vec{X}_{\alpha}) < f(\vec{X}_{best})$ }
		\State $\vec{X}_{best} = \vec{X}_{\alpha}$
	\EndIf
	\State $a = 2 - \frac{2t}{T}$
	\State $t = t + 1$
\EndWhile \\
